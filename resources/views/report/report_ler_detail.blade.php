	...
	<div class="content" id="content">
		...
		<div class="page_header">
			<div class="pull-left">
				...
				<span class="main-text">Labor Efficiency Ratio(LER)</span>
								...
								<table class="table table-bordered" id="displayTable">
									...
									<tbody>
										...
										@foreach ($list as $item)
											<?php 
												$gross_profit=abs($data[$item->job_number]['revenue'])-$data[$item->job_number]['cost'];
												$hours=(($data_hours[$item->job_number]!=0)?$data_hours[$item->job_number]:1);
												$LER=$gross_profit/$hours;
											?>
											<tr class="table-row tr-bg-success" data-margin="{{number_format($LER,2,'.','')}}">
												<td>{{ $item->job_number }}</td>
												<td>{{ $item->job_description}}</td>
												<td class="text-right">{{number_format(abs($data[$item->job_number]['revenue']),2)}}</td>
												<td class="text-right">{{number_format(abs($data[$item->job_number]['cost']),2)}}</td>
		                                        <td class="text-right">{{number_format($gross_profit,2)}}</td>
		                                        <td class="text-right">{{number_format($data_hours[$item->job_number],2)}}</td>
